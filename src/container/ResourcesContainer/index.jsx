...
import { Book, GraduationCap, Zap, BookOpen, ShieldCheck, Layers, ArrowRight, Clock, User, Star, Film, Library } from 'lucide-react';
...
const ResourcesContainer = () => {
  const [activeTab, setActiveTab] = useState('textbooks'); // 'textbooks' or 'videos'
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-24 sm:pt-32 pb-32 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Header Section */}
        <div className="mb-12 flex flex-col md:flex-row items-start md:items-end justify-between gap-6 border-b border-slate-200 pb-10">
          <motion.div
          ...
          {/* SaaS Tab Navigation Selector */}
          <div className="flex bg-slate-200/60 p-1.5 rounded-[1.5rem] border border-slate-200/50 w-full md:w-auto">
            <button
              onClick={() => setActiveTab('textbooks')}
              className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3.5 text-xs font-black uppercase tracking-wider rounded-2xl transition-all shadow-md ${
                activeTab === 'textbooks'
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'textbooks' ? (
            ...
            <motion.div
              ...
              exit={{ opacity: 0, y: 15 }}
              transition={{ duration: 0.4 }}
              className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8 sm:p-12"
            >
              <VideoSection />
            </motion.div>
          )}
